...
# catGT parameters
DIR = ' -dir=C:/Users/mmustans/Documents/Data/MM001_Sansa/Electrophysiology/2024-04-18'
RUN = ' -run=BSD'
...
xia2 = ' -xia=0,0,5,2.5,3,0'  # stim stop
...
subprocess.run('CatGT' + DIR + RUN + prs + xia + xa + xa2 + xia2, shell=True, capture_output=True)
...
stimstart = utils.getFilePath(windowTitle="SYNC stimulus start (usually xa4)", filetypes=[("CatGT output", "*.txt")])
...
subprocess.run('TPrime' + syncperiod + tostream + fromstream + trialstart + trialstop + stimstart + stimstop + stimstop, shell=True)
